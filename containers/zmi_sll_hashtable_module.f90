...
! Simple implementation of hashtable with singly linked list container (non-generic!)
! Tested but can still contain some bugs!

!-----------------------------------------------------------------------
!Module zmi_singly_linked_list_node_module
!-----------------------------------------------------------------------
module zmi_singly_linked_list_node_module
   implicit none
   private ! all by default
   public :: sl_list_node_type
   ...
   type sl_list_node_type
      integer, private :: key = 0
      integer, private :: val = 0
      type(sl_list_node_type), pointer :: next => null()
   contains
      procedure, pass :: clear => clear_sl_list_node
      procedure, pass :: get_key => get_key_sl_list_node
      procedure, pass :: get_next => get_next_sl_list_node
      procedure, pass :: get_storage_size => get_storage_size_sl_list_node
      procedure, pass :: get_val => get_val_sl_list_node
      procedure, pass :: print => print_sl_list_node
      procedure, pass :: set_key => set_key_sl_list_node
      procedure, pass :: set_next => set_next_sl_list_node
      procedure, pass :: set_val => set_val_sl_list_node
   end type sl_list_node_type
...
contains
   ...
   !Subroutine clear
   !-----------------------------------------------------------------------
   impure elemental subroutine  clear_sl_list_node(this)
      implicit none
      class(sl_list_node_type), intent(inout) :: this
      this % key = 0
      this % val = 0
      this % next => null()
   end subroutine clear_sl_list_node


   !-----------------------------------------------------------------------
   !Function get_storage_size
   !-----------------------------------------------------------------------
   function  get_storage_size_sl_list_node(this) result(size_in_bits)
      implicit none
      class(sl_list_node_type), intent(in) :: this
      integer :: size_in_bits
      size_in_bits = storage_size(this % key) + storage_size(this % val) + storage_size(this % next)
   end function get_storage_size_sl_list_node

   !-----------------------------------------------------------------------
   !Function get_next ! access next node
   !-----------------------------------------------------------------------
   impure function  get_next_sl_list_node(this) result(next_node)
      implicit none
      class(sl_list_node_type), intent(in) :: this
      type(sl_list_node_type), pointer :: next_node
      if(associated(this % next)) then
         next_node => this % next
      else
         next_node => null()
      endif
   end function get_next_sl_list_node

   !-----------------------------------------------------------------------
   !Subroutine set_next ! set next node
   !-----------------------------------------------------------------------
   subroutine set_next_sl_list_node(this, next_node)
      class(sl_list_node_type), intent(inout) :: this
      type(sl_list_node_type), pointer, intent(in) :: next_node
      if(associated(next_node)) then
         this % next => next_node
      else
         this % next => null()
      endif
   end subroutine set_next_sl_list_node

   !-----------------------------------------------------------------------
   !Function get_key
   !-----------------------------------------------------------------------
   pure function  get_key_sl_list_node(this) result(key)
      implicit none
      class(sl_list_node_type), intent(in) :: this
      integer :: key
      key = this % key
   end function get_key_sl_list_node

   !-----------------------------------------------------------------------
   !Subroutine set_key
   !-----------------------------------------------------------------------
   pure subroutine  set_key_sl_list_node(this, key)
      implicit none
      class(sl_list_node_type), intent(inout) :: this
      integer, intent(in) :: key
      this % key = key
   end subroutine set_key_sl_list_node

   !-----------------------------------------------------------------------
   !Function get_val
   !-----------------------------------------------------------------------
   pure function  get_val_sl_list_node(this) result(val)
      implicit none
      class(sl_list_node_type), intent(in) :: this
      integer :: val
      val = this % val
   end function get_val_sl_list_node

   !-----------------------------------------------------------------------
   !Subroutine set_val
   !-----------------------------------------------------------------------
   pure subroutine  set_val_sl_list_node(this, val)
      implicit none
      class(sl_list_node_type), intent(inout) :: this
      integer, intent(in) :: val
      this % val = val
   end subroutine set_val_sl_list_node

   !-----------------------------------------------------------------------
   !Subroutine print
   !-----------------------------------------------------------------------
   subroutine print_sl_list_node(this)
      class(sl_list_node_type), intent(in) :: this
      write(*,*)"------------------------------------------------------------"
      write(*,*)"print_sl_list_node"
      write(*,*)"------------------------------------------------------------"
      write(*,format_int) "key = ", this % key
      write(*,format_int) "val = ", this % val
      write(*,format_logic) "associated(this % next) = ", associated(this % next)
   end subroutine print_sl_list_node

end module zmi_singly_linked_list_node_module

!-----------------------------------------------------------------------
!Module zmi_singly_linked_list_module
!-----------------------------------------------------------------------
module zmi_singly_linked_list_module
   use zmi_singly_linked_list_node_module
   implicit none
   private ! all by default
   public :: sl_list_type
   ...
   type sl_list_type
      private
      type(sl_list_node_type), pointer :: head => null()
      integer :: items_count = 0
   contains
      generic, public :: assignment(=) => assignment_operator_sl_list
      generic, public :: operator(==) => is_equal_to_operator_sl_list
      procedure, pass :: add_first => add_first_sl_list
      procedure, pass :: assignment_operator_sl_list
      procedure, pass :: delete => delete_all_sl_list
      procedure, pass :: delete_first => delete_first_sl_list
      procedure, pass :: delete_key => delete_key_sl_list
      procedure, pass :: export => export_all_sl_list
      procedure, pass :: get_first => get_first_sl_list
      procedure, pass :: get_size => get_size_sl_list
      procedure, pass :: get_storage_size => get_storage_size_sl_list
      procedure, pass :: is_empty => is_empty_sl_list
      procedure, pass :: is_equal_to_operator_sl_list
      procedure, pass :: move => move_sl_list
      procedure, pass :: print => print_all_sl_list
      procedure, pass :: search_key => search_key_sl_list
      final :: finalise_sl_list
   end type sl_list_type

contains

   !-----------------------------------------------------------------------
   !Subroutine is move_sl_list ! move nodes from rhs to this
   !-----------------------------------------------------------------------
   impure elemental subroutine move_sl_list(this,rhs)
      implicit none
      class(sl_list_type), intent(inout) :: this ! LHS
      type(sl_list_type), intent(inout) :: rhs ! RHS
      ...
      if(this_size /= rhs_size) stop "assignment_operator_sl_list"

      rhs % head => null()
      rhs % items_count = 0
   end subroutine move_sl_list
   ...
   !-----------------------------------------------------------------------
   impure elemental function is_equal_to_operator_sl_list(this,rhs) result(is_equal)
      implicit none
      class(sl_list_type), intent(in) :: this ! LHS
      type(sl_list_type), intent(in) :: rhs ! RHS
      logical :: is_equal

      type(sl_list_node_type), pointer :: node1, node2
      integer :: key1, key2
      integer :: val1, val2
   ...
   end function is_equal_to_operator_sl_list


   !-----------------------------------------------------------------------
   !Subroutine is overloaded assignment operator generic, public :: assignment(=) => name
   !-----------------------------------------------------------------------
   impure elemental subroutine assignment_operator_sl_list(this,rhs)
      implicit none
      class(sl_list_type), intent(inout) :: this ! LHS
      type(sl_list_type), intent(in) :: rhs ! RHS
      type(sl_list_node_type), pointer :: new_node
      type(sl_list_node_type), pointer :: this_node
      type(sl_list_node_type), pointer :: rhs_node
      integer :: key
      integer :: val
      integer :: this_size, rhs_size, i
      ...
      rhs_size = rhs % get_size()
      rhs_node => rhs % get_first()

      do i = 1, rhs_size
         key = rhs_node % get_key()
         val = rhs_node % get_val()

         if (this % is_empty()) then
            allocate(this % head)
            call this % head % set_key(key)
            call this % head % set_val(val)
            this % items_count = 1
            this_node => this % head
         else
            allocate(new_node)
      ...
      this_size = this % get_size()
      if(this_size /= rhs_size) stop "assignment_operator_sl_list"
   end subroutine assignment_operator_sl_list


   !-----------------------------------------------------------------------
   !Subroutine finalise_sl_list ! deallocate list (deep)
   !-----------------------------------------------------------------------
   subroutine finalise_sl_list(this)
      type(sl_list_type), intent(inout) :: this
      call this % delete()
   end subroutine  finalise_sl_list

   !-----------------------------------------------------------------------
   !Function get_storage_size_sl_list
   !-----------------------------------------------------------------------
   function  get_storage_size_sl_list(this) result(size_in_bits)
      implicit none
      class(sl_list_type), intent(in) :: this
      integer :: size_in_bits
      type(sl_list_node_type), pointer :: node
   ...
   end function get_storage_size_sl_list


   !-----------------------------------------------------------------------
   !Subroutine get_first ! access first node
   !-----------------------------------------------------------------------
   function  get_first_sl_list(this) result(first_node)
      implicit none
      class(sl_list_type), intent(in) :: this
      type(sl_list_node_type), pointer :: first_node
      first_node => this % head
   end function get_first_sl_list


   !-----------------------------------------------------------------------
   !function is_empty ! test whether sl_list is empty
   !-----------------------------------------------------------------------
   pure function  is_empty_sl_list(this) result(is_empty)
      implicit none
      class(sl_list_type), intent(in) :: this
      logical ::  is_empty
   ...
   end function is_empty_sl_list


   !-----------------------------------------------------------------------
   !Function get_size
   !-----------------------------------------------------------------------
   pure function  get_size_sl_list(this) result(list_size)
      implicit none
      class(sl_list_type), intent(in) :: this
      integer :: list_size
      list_size = this % items_count
   end function get_size_sl_list

   !-----------------------------------------------------------------------
   !Subroutine add_first !  insert node at the beginning
   !-----------------------------------------------------------------------
   pure subroutine  add_first_sl_list(this, key, val)
      implicit none
      class(sl_list_type), intent(inout) :: this
      integer, intent(in) :: key
   ...
   end subroutine add_first_sl_list

   !-----------------------------------------------------------------------
   !Subroutine delete_first !  delete node at the beginning
   !-----------------------------------------------------------------------
   pure subroutine  delete_first_sl_list(this)
      implicit none
      class(sl_list_type), intent(inout) :: this
      type(sl_list_node_type), pointer :: node
   ...
   end subroutine delete_first_sl_list

   !-----------------------------------------------------------------------
   !Subroutine delete
   !-----------------------------------------------------------------------
   pure subroutine  delete_all_sl_list(this)
      implicit none
      class(sl_list_type), intent(inout) :: this
      integer :: items_count, i
   ...
   end subroutine delete_all_sl_list


   !-----------------------------------------------------------------------
   !Subroutine search_key, no duplicates
   !-----------------------------------------------------------------------
   subroutine search_key_sl_list(this, key, val, nindex)
      implicit none
      class(sl_list_type), intent(in) :: this
      integer, intent(in) :: key
      ...
      do i = 1, items_count
         ...
         node => node % get_next()
      enddo
   end subroutine search_key_sl_list


   !-----------------------------------------------------------------------
   !Subroutine delete_key  ! no search for duplicates ?  TODO!!!
   !-----------------------------------------------------------------------
   subroutine delete_key_sl_list(this, key, val, nindex)
      implicit none
      class(sl_list_type), intent(inout) :: this
      integer, intent(in) :: key
      integer, intent(out) :: val
      ...
      integer :: items_count
      integer :: i
      type(sl_list_node_type), pointer :: prev
      type(sl_list_node_type), pointer :: node
      ...
      do i = 1, items_count
         ...
         if(node % get_key() == key) then
            ...
            if(nindex == 1) then
               ...
               this % items_count = this % items_count - 1
               deallocate(node)
            else ! 1 < nindex < items_count, so prev is also associated now
               prev % next => node % next
               this % items_count = this % items_count - 1
               deallocate(node)
            endif
         ...
         node => node % get_next()
      enddo
   end subroutine delete_key_sl_list


   !-----------------------------------------------------------------------
   !Subroutine export ! export all keys, values of the list
   !-----------------------------------------------------------------------
   subroutine  export_all_sl_list(this, keys, values)
      implicit none
      class(sl_list_type), intent(in) :: this
      ...
      if (this % is_empty()) then
         ...
         items_count = this % get_size()
         !          if(size(keys) < items_count) stop "export_all_sl_list"
         !          if(size(values) < items_count) stop "export_all_sl_list"
         node => this % get_first()
         do i = 1, items_count
            keys(i) = node % get_key()
   ...
   end subroutine export_all_sl_list


   !-----------------------------------------------------------------------
   !Subroutine print ! print all nodes of the list
   !-----------------------------------------------------------------------
   subroutine  print_all_sl_list(this)
      implicit none
      class(sl_list_type), intent(in) :: this
      type(sl_list_node_type), pointer :: node
      integer :: items_count, i
   ...
   end subroutine print_all_sl_list

end module zmi_singly_linked_list_module
...
module zmi_hashtable_module
   use, intrinsic :: iso_fortran_env
   use zmi_hash_functions_library_module
   use zmi_singly_linked_list_module
   ...
   type zmi_hashtable_type
      ...
      type(sl_list_type), dimension(:), allocatable :: table ! hashtable
      type(sl_list_type), dimension(:), allocatable :: rtable ! aux hashtable for resize
...
contains
   ...
   subroutine  print_zmi_hashtable(this, print_table)
      ...
      if(present(print_table)) then
         if(print_table) then
            do i = 1, this % ne_max
               if(.not. this % table(i) % is_empty()) then
                  write(*,*)
                  write(*,format_int) "not empty this % table("//int2char(i)//"), i = ", i
...
contains
   ...
   subroutine  test_delete_random_with_reheash(ind, t)
      ...
      do i = 1, hsize
         keys(i) = i !get_random_pos(hsize*10)
      enddo
